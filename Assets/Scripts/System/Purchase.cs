...
public class Purchase : MonoBehaviour
{
        ...
        void Update()
    {
        ...
        if (Select_Purchase.page == 2)
        {
            for (int i = 0 + 6; i < goodsize + 6; i++)
            {
                good[i - 6].transform.GetChild(0).GetComponent<Image>().sprite = Goods[i].img;
                good[i - 6].transform.GetChild(1).GetComponent<Text>().text = Goods[i].name;
            }
        }
    }
}
