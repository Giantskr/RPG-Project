...
public abstract class Select : MonoBehaviour
{
	...
	protected void selection()
	{
		if (Input.GetButtonDown("Horizontal") || Input.GetButtonDown("Vertical"))   //要用GetButtonDown而非GetButton，防止按键持续生效
		{
			...
			if ((Input.GetAxis("Vertical") < 0 || Input.GetAxis("Horizontal") > 0) && states < OptionNum) states++;
			else if ((Input.GetAxis("Vertical") > 0 || Input.GetAxis("Horizontal") < 0) && states > 1) states--;
